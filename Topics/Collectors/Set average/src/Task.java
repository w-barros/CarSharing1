...
class Main {
  ...
  public static Double avgOnSet(Stream<Integer> numbers) {
    Set<Integer> result = numbers.collect(Collectors.toSet());
    OptionalDouble resultDouble = result.stream().mapToDouble(num -> (double) num).average();

    return resultDouble.getAsDouble();
